...
describe('Variables', () => {
	...
	test('set non existent variable', () => {
	...
	test('temporaries at global scope', () => {
	...
	test('variable get set api', () => {
		...
		expect(() => {
			...
			// be what we want.
			story.variablesState["x"] = Map();

		}).toThrow();
	});

	test('variable pointer ref from knot', () => {
		loadStory("variable_pointer_ref_from_knot");

		expect(story.Continue()).toBe("6\n");
	});

	test('variable swap recurse', () => {
